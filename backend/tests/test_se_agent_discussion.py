"""Tests for the fishery SE agent's operationalization discussion: after a
proposal, the proposing agent and the SE agent go back and forth for a
...
from genfishery.se_agent.client import ImplementResult
...
class FakeSEAgentClient:
    def __init__(self) -> None:
        self.sessions_started: list[str] = []
        self.asks: list[tuple[str, str]] = []
        self.implement_calls: list[tuple[str, str]] = []
    ...
    async def ask(self, session_id: str, message: str) -> str:
        ...
        return f"se agent question about: {message[:20]}"

    async def implement_norm(self, session_id: str, instructions: str) -> ImplementResult:
        self.implement_calls.append((session_id, instructions))
        return ImplementResult(success=True, reply="done", commit_sha="deadbeef")
...
async def test_discussion_phase_runs_configured_rounds_and_returns_final_reply():
    ...
    assert len(se_agent.sessions_started) == 1
    # 1 opening ask + (rounds - 1) follow-up asks, since the final turn's
    # reply is finalized rather than relayed back to the SE agent.
    assert len(se_agent.asks) == 3
    assert len(decisions.reply_calls) == 3
    assert [c["is_final_turn"] for c in decisions.reply_calls] == [False, False, True]
    # Transcript grows by one (se_agent, agent) pair each turn.
...
async def test_propose_phase_uses_se_agent_to_finalize_operationalization():
...
async def test_propose_phase_leaves_operationalization_blank_without_se_agent():
